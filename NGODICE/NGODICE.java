...
public class NGODICE{
	...
	private String[] score_name = {"S", "D", "G", "K"};
	private String[] face = {"さ","だ","げ","か","ご","ん"};
	private int[] face_point = {500, 500, 500, 500, 100, 50};
	//単語ごとの文字必要数
	private int[][] word = {{1, 0, 0, 0, 1, 1}, //さんご
							{0, 1, 0, 0, 1, 1}, //たんご
							{0, 0, 1, 0, 1, 1}, //げんご
							{0, 0, 0, 1, 1, 1}, //かんご
							{1, 0, 0, 1, 0, 2}, //さんかん
	...
	private String[] words = {"SANGO","DANGO","GENGO","KANGO","SANKAN","DANGEN","GENKAN","KANDANSA"};
	private int[] word_point = {1000, 1000, 1000, 1000, 3000, 3000, 3000, 100000};
	public NGODICE(){
		//nothing
	}
	public void information(){
		print("これはNGODICEです", 2);
		if(question("簡単な説明を聞きますか？")){
			print("ダイスを振って高得点を目指すゲームです", 2);
			print("お椀を振る（ロール）は基本3回です", 2);
			print("ダイスは基本5つです", 2);
			print("目は、「さ」「だ」「げ」「か」「ご」「ん」です", 2);
	...
	public void result(){
		...
		print("", 1);
		print("S：" + score[0] + " / D：" + score[1], 0.5f);
		print("G：" + score[2] + " / K：" + score[3], 1);
	...
	public void judge(){
		...
		for(int i = 0; i < score.length; i++) score[i] += (n_of_face[i] * face_point[i]) + (n_of_face[4] * face_point[4]) + (n_of_face[5] * face_point[5]);
				...
				if(word_combo[i] > 0){
					...
					switch(i){
						case 4:
							...
							score[3] += word_point[i] + word_point[i] * pow(2, word_combo[i] - 1);
							break;
						case 5:
						...
						//0,1,2,3
						default:
							score[i] += word_point[i] + word_point[i] * pow(2, word_combo[i] - 1);
					}
				}
			}
			n_of_roll++;
